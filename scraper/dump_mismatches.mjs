...
if (!DATABASE_URL) {
  console.error(
    "DATABASE_URL not set. Run: node --env-file=.env.local scraper/dump_mismatches.mjs",
  );
  process.exit(1);
}
...
for (const m of mismatches) {
  const tag =
    m.overlap > 0.9 ? "SAME hadith (trivial diff)" : "DIFFERENT hadith (numbering mismatch)";
  w(`### (${m.collection}, ${m.hadithNumber})  arabicURN=${m.arabicURN}`);
  w(
    `VERDICT        : ${tag}   [overlap ${(m.overlap * 100).toFixed(0)}%, CSV=${m.csvLen} letters, DB=${m.dbLen} letters]`,
  );
