...
A single denormalized table **`public.hadith_table`** — a 1:1 mirror of the sunnah-db MariaDB dump, so columns are **quoted camelCase** (`"arabicURN"` PK, `"bookNumber"` *varchar*, `"hadithNumber"` *varchar*, may be comma-joined like `"521, 522"`, `"ourHadithNumber"` int, `"englishText"`, `"arabicText"`, `"englishBabName"`, `"arabicBabName"`, `"englishgrade1"`, `collection`). Two **nullable Urdu columns** (`"urduText"`, `"urduSanad"`) were added later (scraper PR + `0017_urdu_columns_and_rpcs.sql`) and populated out-of-band from sunnah.com — every row-producing RPC projects them as `urdu_text`/`urdu_sanad`, which the mapper combines into `text_ur` (isnad + matn). RLS = read-all. Embeddings live separately in `hadith_embeddings` (`arabic_urn` PK, `halfvec(1024)`, HNSW cosine, `model`/`text_hash`). Other tables: `query_cache`, `search_logs`, `feedback`, `bookmarks`.
...
- **Trilingual *display* ships; multilingual *search* is still deferred.** Every hadith now renders in Arabic + English + **Urdu** (`text_ur`, surfaced on detail/search/browse cards via a `showUrdu` toggle mirroring `showArabic`; Nastaliq `.font-urdu` on web, Amiri/Naskh on mobile). But **semantic/keyword retrieval is still English-embedded + bukhari-only** — Urdu is a display translation, not a search target. Reaching free-text situation queries across collections + languages still needs, roughly: per-`(arabic_urn, lang)` embeddings (the PK is `arabic_urn` alone today), bilingual/Urdu embedded passages (ingest currently embeds English-only), a `collection_filter` param on the bukhari RPCs (or generalized `search_hadith_hybrid`), per-language `ts_config`/GIN indexes, and query-language detection (today `req.language` only namespaces the cache). Do those when search (not display) across collection #2 / language #3 is actually scheduled.
- **Urdu is overlaid, not seeded.** `"urduText"`/`"urduSanad"` are populated out-of-band by the scraper; `scripts/load_chunks.mjs` **truncates + reloads `hadith_table` from seed (no Urdu)**, so a corpus reload wipes them — re-run the Urdu populate after any `load_chunks`. `0017`'s `ADD COLUMN IF NOT EXISTS` is idempotent, so it's safe regardless of the scraper PR's own `0013_add_urdu_columns.sql`.
